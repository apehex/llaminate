...
class TransformerTest(tf.test.TestCase):
    ...
    def test_shapes(self):
        ...
        # call
        __y, __c = __m(inputs=__x, cache=__c, mask=None, position=4, training=False)
        # checks
    ...
    def test_cache_ignored_during_training(self):
        """Test with a mask tensor."""
        __meta = {
            'num_layers': 2,
            'num_heads': 4,
        ...
        __x = tf.ones((__meta['batch_dim'], math.prod(__meta['token_dim']) * __meta['cache_dim'], __meta['embed_dim']))
        # call
        __y = __m(inputs=__x, cache=__c, mask=None, position=0, training=True)
        # checks
        assert isinstance(__y, tf.Tensor) # not a tuple like (y, cache)
        self.assertEqual(__y.shape, __x.shape)
